...
%neutralizing ions.
ions1=calccharge(species1,-qm1,dt,dx,lx,n1,wp1,ng);
ions2=calccharge(species2,-qm2,dt,dx,lx,n2,wp2,ng);
ioncharge=ions1+ions2;
...
for i=1:nt
	...
	if (i==1 || i==100 || i==nt || i==500)
		...
		xlabel('x');
		ylabel('Charge Density q/m^2');	
		sv=strcat(name,'Val_weakBeam_PhaseECharget',num2str(i));
		saveas(h,sv);
...
ylabel('Energy (J)');
xlabel('time (s)');
saveas(h,strcat(name,'Val_weakBeam_Efieldgrowth'));
saveas(h,strcat(name,'Val_weakBeam_Efieldgrowth.epsc2'));
...
ylabel('Energy (J)');
xlabel('time (s)');
saveas(h,strcat(name,'Val_weakBeam_EnergyEvolution'));
saveas(h,strcat(name,'Val_weakBeam_EnergyEvolution.epsc2'));
...
xlabel('time (s)');
saveas(h,strcat(name,'Val_weakBeam_QuantitiesEvolution'));
saveas(h,strcat(name,'Val_weakBeam_QantitiesEvolution.epsc2'));
